...
(function() {
    ...
    const i18n = new Map(Object.entries({
        ...
        // --- UI 界面文本 ---
        'See what the Community is dreaming up.': '看看社区正在构想些什么。',
        'Explore': '探索',
        'Image Gallery': '图像库',
        'Browse and share Image Generations from the NovelAI community.': '浏览并分享来自 NovelAI 社区的生成图像。',
        'Register for NovelAI Explore to like and upload': '注册 NovelAI Explore 即可点赞和上传',
        'Register': '注册',
        'The Image Gallery': '图像库',
        'See what the NovelAI Community is dreaming up.': '看看 NovelAI 社区正在构想些什么。',
        ...
        'Local Changes': '本地更改',
        'Last Remote Save': '上次云端保存',
        'Title': '标题',
        'Last Changed': '最后修改',
        'Recent Story': '最近的故事',
        'There are updates available!': '有可用更新！',
        'You\'re not currently experiencing the best NovelAI has to offer! Updating by refreshing the page is recommended.': '你目前体验的不是NovelAI提供的最佳版本！建议通过刷新页面进行更新。',
        'Make sure you save everything you want to keep before you do. Refreshing the page will clear any generated images.': '在刷新之前，请确保保存了所有你想保留的内容。刷新页面将清除所有已生成的图像。',
        'OK': '确定',
        'See Updates': '查看更新',
        'Drop image files here, or click to select files.': '将图像文件拖放至此，或点击选择文件。',
        'Only raw images generated by NovelAI can be accepted.': '仅接受NovelAI生成的原始图像。',
        'Post Title': '帖子标题',
        'Give your image a creative title': '为你的图像起一个有创意的标题',
        'Upload Guidelines': '上传指南',
        'All posts currently undergo manual review and may be approved or rejected at our discretion.': '目前所有帖子均需进行人工审核，我们保留批准或拒绝的权利。',
        'No NSFW.': '禁止NSFW内容。',
        'No Copyright/Character/Artist tags or content.': '禁止版权/角色/画师标签或内容。',
        'Character Reference, Image-to-Image, and Inpainting are not allowed.': '不允许使用角色参考、图生图和局部重绘。',
        'Vibe Transfer is allowed, but may be rejected upon review.': '允许使用氛围转移，但在审核时可能会被拒绝。',
        'Upload Image': '上传图像',
        'Only images generated by NovelAI can be uploaded.': '仅允许上传NovelAI生成的图像。',
        ...
        // --- 图像操作与举报 ---
        'Use Prompt': '使用提示词',
        'Filter by creator': '按创作者筛选',
        'Collapse': '折叠',
        'Download': '下载',
        'Report': '举报',
        'Report Post': '举报帖子',
        'Please select a reason for reporting this post.': '请选择举报此帖子的原因。',
        'Prohibited Content': '违规内容',
        'Copyright': '版权问题',
    ...
    const regexI18n = [
        ...
        { regex: /^Are you sure you want to delete the category ["'](.*?)["']\?$/, replacement: '确定要删除类别 "$1" 吗？' },
        { regex: /^Are you sure you want to trim the history of ["'](.*?)["']\?$/, replacement: '确定要修剪 "$1" 的历史记录吗？' },
        ...
        { regex: /^(\d+\/\d+) text generations remaining\.$/, replacement: '剩余 $1 次文本生成。' },
        { regex: /^Context: (\d+) tokens \(max tokens \[(\d+)\] - output length \[(\d+)\] - continue to sentence end allowance \[(\d+)\]\)$/, replacement: '上下文: $1 Token (最大 Token [$2] - 输出长度 [$3] - 句末补全余量 [$4])' },
        { regex: /^max tokens \[(\d+)\] - output length \[(\d+)\] - continue to sentence end allowance \[(\d+)\]$/, replacement: '最大 Token [$1] - 输出长度 [$2] - 句末补全余量 [$3]' },
        { regex: /^option (.*?), selected\.$/, replacement: "已选择选项 $1。" },
        { regex: /^You are currently focused on option (.*?) Select a (.*?) is focused , press Down to open the menu,$/, replacement: "当前聚焦于选项 $1。已聚焦 $2，按向下键打开菜单，" },
        { regex: /^You are currently focused on option (.*?) (\d+) results available\.(.*)$/, replacement: "当前聚焦于选项 $1，有 $2 个可用结果。$3" },
        { regex: /^This prompt is using (\d+) of the currently used (\d+) tokens\. Max total tokens: (\d+)$/, replacement: "此提示词使用了当前 $2 个 Token 中的 $1 个。最大总容量: $3" },
        { regex: /^(\d+) tokens out of (\d+) tokens used$/, replacement: "已使用 $1 / $2 个 Token" },
        ...
        // ==========================================
        // 1. 纯时间动态匹配
        { regex: /^a few seconds ago$/, replacement: '几秒钟前' },
        { regex: /^a minute ago$/, replacement: '一分钟前' },
        { regex: /^an hour ago$/, replacement: '一小时前' },
        { regex: /^a day ago$/, replacement: '一天前' },
        { regex: /^(\d+) seconds? ago$/, replacement: '$1 秒前' },
        { regex: /^(\d+) minutes? ago$/, replacement: '$1 分钟前' },
        { regex: /^(\d+) hours? ago$/, replacement: '$1 小时前' },
        { regex: /^(\d+) days? ago$/, replacement: '$1 天前' },
        { regex: /^(\d+) months? ago$/, replacement: '$1 个月前' },
        { regex: /^(\d+) years? ago$/, replacement: '$1 年前' },

        // 2. 带有 "Last Edited: " 前缀的动态匹配
        { regex: /^Last Edited: a few seconds ago$/, replacement: '最后编辑：几秒钟前' },
        { regex: /^Last Edited: a minute ago$/, replacement: '最后编辑：一分钟前' },
        { regex: /^Last Edited: an hour ago$/, replacement: '最后编辑：一小时前' },
        { regex: /^Last Edited: a day ago$/, replacement: '最后编辑：一天前' },
        { regex: /^Last Edited: (\d+) seconds? ago$/, replacement: '最后编辑：$1 秒前' },
        { regex: /^Last Edited: (\d+) minutes? ago$/, replacement: '最后编辑：$1 分钟前' },
        { regex: /^Last Edited: (\d+) hours? ago$/, replacement: '最后编辑：$1 小时前' },
        { regex: /^Last Edited: (\d+) days? ago$/, replacement: '最后编辑：$1 天前' },
        { regex: /^Last Edited: (\d+) months? ago$/, replacement: '最后编辑：$1 个月前' },
    ...
    // 编辑器容器选择器，极其重要：防止汉化脚本误替换用户正在输入的文本，造成光标跳动或框架报错
    ...
    function translateTextNode(node) {
        ...
        // 多行空白符归一化为单空格，匹配带换行渲染的英文悬浮提示
        let normalizedText = originalText.replace(/\s+/g, ' ').trim();

        // O(1) 查找
        let exactMatch = i18n.get(normalizedText);
        if (exactMatch) {
        ...
        for (let i = 0; i < regexI18n.length; i++) {
            ...
            if (item.regex.test(normalizedText)) {
                let replaced = normalizedText.replace(item.regex, item.replacement);
    ...
    function translateAttributes(el) {
        if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA') {
            ...
            if (placeholder) {
                let match = i18n.get(placeholder.replace(/\s+/g, ' ').trim());
        ...
        if (el.hasAttribute('title')) {
            ...
            if (title) {
                let match = i18n.get(title.replace(/\s+/g, ' ').trim());
